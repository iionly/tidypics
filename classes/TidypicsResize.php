<?php
/**
 * Elgg Tidypics class of resizing functions
 *
 * @package TidypicsImageResize
 */

class TidypicsResize {
	/**
	...
	*/
	public static function tp_create_gd_thumbnails($file, $prefix, $filestorename) {
	...
	*/
	public static function tp_gd_resize($input_name, $output_name, $watermark, $maxwidth, $maxheight, $square = false, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0) {
		...
		$params = self::tp_im_calc_resize_params($width, $height, $maxwidth, $maxheight, $square, $x1, $y1, $x2, $y2);
		if (!$params) {
			return false;
		}
		...
		if ($watermark) {
			TidypicsWatermark::tp_gd_watermark($newimage);
		}
	...
	* @return bool    TRUE on success
	*/
	public static function tp_create_imagick_thumbnails($file, $prefix, $filestorename) {
		$image_sizes = elgg_get_plugin_setting('image_sizes', 'tidypics');
		if (!$image_sizes) {
			return false;
		}
		...
		$thumb->setFilename($prefix . "thumb" . $filestorename);
		$thumbname = $thumb->getFilenameOnFilestore();
		$rtn_code = self::tp_imagick_resize(
			$file->getFilenameOnFilestore(),
			$thumbname,
			$image_sizes['tiny_image_width'],
			$image_sizes['tiny_image_height'],
		...
		TidypicsWatermark::tp_imagick_watermark($thumbname);
	...
	* @return bool TRUE on success
	*/
	public static function tp_imagick_resize($input_name, $output_name, $maxwidth, $maxheight, $square = false, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0) {
		// Get the size information from the image
		$imgsizearray = getimagesize($input_name);
		if (!$imgsizearray) {
			return false;
	...
	* @return bool    TRUE on success
	*/
	public static function tp_create_im_cmdline_thumbnails($file, $prefix, $filestorename) {
		$image_sizes = elgg_get_plugin_setting('image_sizes', 'tidypics');
		if (!$image_sizes) {
			return false;
		}
		...
		$thumb->setFilename($prefix . "thumb" . $filestorename);
		$thumbname = $thumb->getFilenameOnFilestore();
		$rtn_code = self::tp_im_cmdline_resize(
			$file->getFilenameOnFilestore(),
			$thumbname,
			$image_sizes['tiny_image_width'],
			$image_sizes['tiny_image_height'],
		...
		TidypicsWatermark::tp_im_cmdline_watermark($thumbname, $thumb->container_guid);
	...
	* @return bool
	*/
	public static function tp_im_cmdline_resize($input_name, $output_name, $maxwidth, $maxheight, $square = false, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0) {
		// Get the size information from the image
		$imgsizearray = getimagesize($input_name);
		if (!$imgsizearray) {
			return false;
		...
		$params = self::tp_im_calc_resize_params($orig_width, $orig_height, $maxwidth, $maxheight, $square, $x1, $y1, $x2, $y2);
		if (!$params) {
			return false;
		}
	...
	* @return array|FALSE
	*/
	public static function tp_im_calc_resize_params($orig_width, $orig_height, $new_width, $new_height, $square = false, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0) {
		// crop image first?
		$crop = true;
