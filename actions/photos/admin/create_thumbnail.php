...
if (!($image instanceof TidypicsImage)) {
	return elgg_error_response(elgg_echo('tidypics:thumbnail_tool:unknown_image'), REFERER);
}
...
if (!$filename || !($album instanceof TidypicsAlbum)) {
	return elgg_error_response(elgg_echo('tidypics:thumbnail_tool:invalid_image_info'), REFERER);
}
